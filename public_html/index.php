   ...
   //   if (date == "20180513") { 
      ...
      //          markers[j] = new google.maps.Marker({
            ...
            markers[j].addListener('click', function(j) {
              if (infowindows[this.j].getMap()) { infowindows[this.j].close(map, markers[this.j]);  }
              else { infowindows[this.j].open(map, markers[this.j]); }
             }.bind({j:j})
            );
      
          //  alert(title + time + description + location + lat + lng);        
          }
      //        console.log('dateElements[i]: ', dateElements[i].innerHTML);
        }
        for (var i = 0; i < dateElementsTextArr.length; i++) {
        //  alert(dateElementsTextArr[i][0])
        }
     // }
      }
...
//                echo '<th class="dateselect" id="s'.$date.'" bgcolor="LightGray">'.$date.' <sub>'.$day.'</sub></th>';
          ...
          <script>
            ...
            Array.prototype.forEach.call(datebtns, function(datebtn) {
             ...
             // alert("Clicked" + this.dateclicked);
            }.bind({dateclicked: datebtn.textContent.split(' ')[0]})
