...
function crawlHvOne() {  
 ...
 // var dateToday = today.getFullYear()+''+(today.getMonth()+1)+today.getDate();
  ...
  request(PAGE_URL, function(error, response, body) {
    ...
    var eventCount=0;
    var dateCount=0;
    //var divText = $('html > body > #page > #content > #primary > #main').find('div.'+SEARCH_DIV).text(); // .toLowerCase();
 //   $('html > body > #page > #content > #primary > #main').find('div.'+SEARCH_DIV+' > p').each(function (index, element) {
   ...
   //   console.log($(element).attr('class') ) ; // + ' ' +   $(element).attr('class').substring(0,2));
  ...
  //    console.log(date);
      ...
      geocode_result = googleMapsClient.geocode({
        address: location+" NY"
         ...
         if (err == "timeout") { console.log("Error geocoding: " + err + " Check quota at https://console.developers.google.com/google/maps-apis/apis/geocoding-backend.googleapis.com/quotas?project=pyeventmap&duration=PT1H ")}
         else if (err) { console.log("Error geocoding: " + err)}
        }.bind( {date: date, time: time,   title: title,   description: description, location: location})
     );
...
/*
  //  divList.forEach(function(element){
    for (i=0; i< divList.length; i++) {
      ...
      var time = eventhtml.substring(eventhtml.indexOf('\'')+1, eventhtml.indexOf('<strong>')).trim();
      var date = SEARCH_DIV.insert(4, "-").insert(7,'-');
      console.log(date);
      var title = $(element).find('a').text();
 //     var location = eventhtml.substring( eventhtml.lastIndexOf('\n')+1).replace('.','').trim();
      var location = he.decode(eventhtml.substring( eventhtml.lastIndexOf('\n')+1).replace('.','').trim());
      console.log ("LOCATION: " + location);
     ...
     // location = eventhtml.split('\n')[1];
      ...
      geocode_result = googleMapsClient.geocode({
        address: location+" NY"
      }, function(err, response, date, time, title, description, location, i) {
          if (!err) {
           lat =  response.json.results[0].geometry.location.lat; 
           lng =  response.json.results[0].geometry.location.lng;
           addEvent( this.date, this.time,  this.title,  this.description, this.location, lat, lng);
        ...
        }.bind( {date: date, time: time,   title: title,   description: description, location: location, i: i })
     );
    };
*/
  //  console.log(bodyText);
    //console.log(divText);
    //console.log("Found " + divList.length + " events for " + date )
 //   console.log("Found " + eventCount + " events for " + date )
