...
var googleMapsClient = require('@google/maps').createClient({
  key: 'AIzaSyB_WHjYvTLQwOcmvb30qbkwp1jO-xPVySk'
});
...
var con = mysql.createConnection({
  host: "localhost",
  user: "hveventm_crawler", // "root",
  password: "fuckSpiderBites", // "hello123",
  database: DBNAME
});
...
function crawlKingstonHappeningsEvents() {
  ...
  fs.readdir(path, function(err, items) {
    for (var i = 0; i < items.length; i++) {
      var PAGE_URL = "http://kingstonhappenings.org/events/" + items[i];
  ...
  request(PAGE_URL, function(error, response, body) {
    ...
    $('html').find('div.entry-content').each(function(index, element) {
      ...
      $(element).find('p').each(function(index, subelement) {
        //      console.log("---------p FOUND ---------");
        //      console.log($(subelement).html())
        //      var title = $(subelement).text();
        //      console.log(title);
      });
